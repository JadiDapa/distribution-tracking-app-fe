...
export default function AccountDetail() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setStatus("success");
      setMessage("Profile Picture Successfully Changed Sent!");
    } catch (error) {
      setStatus("error");
  ...
  if (account && materials && tools && vehicles && requests) {
    return (
        ...
        <div className="flex w-full flex-col gap-6 lg:flex-row">
          <div className="box-shadow flex w-full flex-col gap-6 rounded-md bg-white p-6 lg:w-[70%] lg:flex-row">
            <div className="flex flex-col items-center justify-center gap-3">
              <div className="group relative w-28 overflow-hidden rounded-md border-[3px]">
                <img
                  className="w-full"
                  src={
                    account?.picture ||
                ...
                />
                {(userData?.id === 1 ||
                  userData?.id === parseInt(accountId as string)) && (
                  <>
                    <div className="absolute left-0 top-0 flex h-full w-full items-center justify-center gap-2 bg-black/40 text-sm text-white opacity-0 transition-all duration-300 group-hover:opacity-100">
                      <div>Change</div>
                      <Pencil size={16} />
                    </div>
                    <Input
