...
import { UpdateToolInvetory } from "@/lib/network/useToolInventory";
import ToolQuantityUpdater from "@/components/Request/ToolQuantityUpdater";
...
export default function ToolInventoryUpdate() {
  const { requestedItems, clearItem } = useRequestItemStore();
  const { userData } = useAuthStore();
  const { updateInventory } = UpdateToolInvetory();
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      setStatus("success");
      setMessage("Tool Quantities Updated!");
      clearItem();
  ...
  if (requestedItems) {
    return (
      <section className="flex w-full flex-col gap-6 py-6">
        <SeactionHeader section="Modify" subSection="Tool Quantity" />
        <Form {...form}>
          <form
