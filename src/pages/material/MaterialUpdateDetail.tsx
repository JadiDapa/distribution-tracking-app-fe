...
import { GetMaterialUpdateById } from "@/lib/network/useMaterialUpdates";
import MaterialUpdateDetailTable from "@/components/Material/MaterialUpdateDetailTable";
import { requestDetailColumn } from "@/utils/table/request-detail";

export default function MaterialUpdateDetail() {
  const { updateId } = useParams();
  const { update, isLoading, isError } = GetMaterialUpdateById(updateId);
  ...
  if (update) {
    return (
      <section className="flex w-full flex-col gap-6 py-6">
        <SeactionHeader
          section="Material"
          subSection="Materiala Update Detail"
        />
        <div className="">
